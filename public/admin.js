    ...
    try {
        ...
        if (pBody) {
            pBody.innerHTML = data.products.map(p => `
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="p-3 text-teal-700 font-semibold">$${p.price.toFixed(2)}</td>
                    <td class="p-3"><span class="px-2 py-1 bg-teal-100 text-teal-700 rounded text-xs">${p.category}</span></td>
                    <td class="p-3">
                        <button onclick="deleteProduct('${p._id}')" class="text-red-500 hover:text-red-700">
        ...
        if (oBody) {
            oBody.innerHTML = data.orders.map(o => `
                <tr class="hover:bg-gray-50">
                    <td class="p-3"><strong>${o.userName}</strong><br><small>${o.phone || "No Phone"}</small></td>
                    <td class="p-3 text-sm max-w-xs">${o.address || "No Address"}</td>
                    <td class="p-3 text-sm">${o.items.map(i => i.name).join(", ")}</td>
                    <td class="p-3 font-semibold text-teal-700">$${o.total.toFixed(2)}</td>
                    <td class="p-3"><span class="px-2 py-1 bg-yellow-100 text-yellow-700 rounded text-xs">${o.status || "Pending"}</span></td>
                </tr>
            `).join("");
        }

        // SALES CHART
        if (data.orders.length > 0) drawChart(data.orders);

        // Notify homepage to reload products
...
function drawChart(orders) {
    ...
    for (let date in grouped) chartData.push([date, grouped[date]]);
