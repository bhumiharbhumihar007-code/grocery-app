...
function createProductCard(product){
    ...
    <div class="bg-white rounded-2xl shadow-md hover:shadow-xl transition overflow-hidden">
        ...
        <div class="p-4">
            <span class="text-xs bg-teal-700 text-white px-2 py-1 rounded">${product.category}</span>
            <h3 class="font-bold text-lg mt-2">${product.name}</h3>
            <p class="text-copper font-bold text-xl">₹${product.price.toFixed(2)}</p>
...
function showToast(message){
    ...
    toast.className = "fixed bottom-5 right-5 bg-teal-700 text-white px-6 py-3 rounded-lg shadow-lg";
...
document.addEventListener("DOMContentLoaded",()=>{
    ...
    // If admin added a product, reload homepage
    window.addEventListener("storage", (e)=>{
        if(e.key==="reloadProducts") loadProducts();
    });
});
